...
def evaluate_lear_in_test_dataset(path_datasets_folder=os.path.join('.', 'datasets'), 
                                  ...
                                  dataset='FR', years_test=1, calibration_window=364 * 3, 
...
forecast = evaluate_lear_in_test_dataset(path_datasets_folder=os.path.join('.', 'datasets'), 
                                  path_recalibration_folder=os.path.join('.', 'experimental_files'),
                                  dataset='FR', years_test=1, calibration_window=364 * 3, 
                                  begin_test_date=None, end_test_date=None)
...
def evaluate_lear_in_test_dataset(path_datasets_folder=os.path.join('.', 'datasets'), 
    ...
    Xtrain, Ytrain, Xtest, = model._build_and_split_XYs(df_train=df_train, df_test=df_test, date_test=None)

    # Recalibrating the LEAR model and extracting the prediction
    Yp = model.recalibrate_predict(Xtrain=Xtrain, Ytrain=Ytrain, Xtest=Xtest)


    Yp = model.recalibrate_predict(df=data_available, next_day_date=date, 
                                                    calibration_window=calibration_window)
    # Saving the current prediction
    forecast.loc[date, :] = Yp
